...
async def execute_bench(url, record_count=5000):
    ...
    async with SQLClient(url) as client:
        ...
        if (await client.execute("drop table pony;")).success:
            print("Table pony is deleted. Crashed previous time? Clean up didn't happen.")
        else:
            print("Table pony does not exist. That's expected.")

        # Create table
        with Bench("create table pony"):
            await client.execute("create table pony (id INTEGER PRIMARY KEY AUTOINCREMENT, name, age, color)")
        ...
        tasks = []
        with Bench(f"preparing insertion of {record_count} records by creating {record_count} tasks with insert statement."):
        ...
        with Bench("waiting until all inserted data is available for read") as bench:
            while True:
                records_inserted = (await client.execute("SELECT count(0) as c from pony", debug=True)).records[0]["c"]
                print("Records insert check: {}/{}".format(records_inserted,record_count))
...
async def async_test(url):
    async with SQLClient(url) as client:
        await client.execute("create table insert_test (id INTEGER PRIMARY KEY AUTOINCREMENT, description)")
        await client.execute("insert into insert_test(description) VALUES (?)", ("test",))
...
async def main():
    ...
    with Bench("checks synchronization speed server"):
        for x in range(1,11):
            print("Check {} if inserted record to server is directy available: {}".format(x, await async_test(url_server)))
    ...
    with Bench("checks synchronization speed file"):
        for x in range(1, 11): 
            print("Check {} if inserted record to file is directy available: {}".format(x, await async_test(url_file)))
    input("Press enter to continue")
